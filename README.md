# Vectra Stream Google SecOps Integration
...
1. Setup Google Security Operations Forwarder.
    - Users must first install and configure the Google Security Operations forwarder for Vectra Stream log type in their environment. Refer to the below guides for detailed setup instructions.
...
2. Configure Vectra for Syslog Forwarding
    - Once the Google SecOps Syslog forwarder is configured, Vectra Administrator users can enable Vectra to send the vectra network metadata information over syslog to external collectors for further storage and analysis.
...
4. Select Vectra Stream log source from the Log Source list.
...
3. Filter Events by Log Type:
    ...
    - Apply a filter for the metadata field "log_type". Set the filter value to metadata.log_type="VECTRA_STREAM".
